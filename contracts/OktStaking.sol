...
contract OktStaking is Ownable {
    ...
    struct PoolInfo {
        ...
        uint256 accCherryPerShare; // Accumulated CHERRYs per share, times 1e12. See below.
    }
    ...
    // The block number when CHERRY mining ends.
    ...
    function pendingReward(address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 cherryReward = multiplier.mul(rewardPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
